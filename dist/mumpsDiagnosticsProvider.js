 ...
 *
 */
class MumpsDiagnosticsProvider {
    constructor(document, collection) {
        this._linetokens = [];
        this._diags = [];
        this._variablesToBeIgnored = [];
        this._enableVariableCheck = true;
        this._subroutines = [];
        ...
        this._activeSubroutine = { name: '', startLine: -1, endLine: -1 };
        ...
        if (document && document.languageId === 'mumps') {
            ...
            for (let i = 0; i < document.lineCount; i++) {
                ...
                const lineInfo = parser.analyzeLine(line.text);
            ...
            for (let i = routine.startLine; i <= routine.endLine; i++) {
                ...
                for (let j = 0; j < this._linetokens[i].length; j++) {
                    ...
                    if (i === routine.startLine && j === 0) { // skip parameters
                        while (++j < this._linetokens[i].length && this._linetokens[i][j].type === mumpsLineParser_1.TokenType.local) { /* empty */ }
                    ...
                    if (token.type === mumpsLineParser_1.TokenType.intendation) {
                    ...
                    else if (token.type === mumpsLineParser_1.TokenType.comment) {
                        if (!intendationFound && level > 0) {
                            this._reduceIntendationLevel(level, 0);
                            level = 0;
                        }
                    }
                    if (token.type === mumpsLineParser_1.TokenType.keyword) {
                        if (!intendationFound && level > 0) {
                            this._reduceIntendationLevel(level, 0);
                            level = 0;
                        }
                        if (token.longName === "NEW") {
                            ...
                            while (++j < this._linetokens[i].length &&
                                (this._linetokens[i][j].type === mumpsLineParser_1.TokenType.local ||
                                    this._linetokens[i][j].type === mumpsLineParser_1.TokenType.sysvariable)) {
                                anyVariablesNewed = true;
                                token = this._linetokens[i][j];
                                if (token.isExcludedVariable) {
                                    containsExclusions = true;
                                }
                                if (token.type === mumpsLineParser_1.TokenType.local) {
                                    const varName = token.name;
                                    let message = '';
                                    const varState = this._varStates[varName] ? this._varStates[varName] : { isExcluded: token.isExcludedVariable };
                    ...
                    else if (token.type === mumpsLineParser_1.TokenType.local) { // local variable found at a non NEW command
                        const varName = token.name;
    ...
    _checkVariableUsage(routine) {
        for (const key in this._varStates) {
            ...
            if (state.isParameter && !state.isUsed && !this._isIgnoredVariable(key)) {
                this._addWarning("Variable " + key + " is a formal parameter but not used", routine.startLine, state.parameterPosition, key.length);
            }
            else if (state.newedAtLine && state.newedAtLine.length > 0 && !state.isUsed && !this._isIgnoredVariable(key)) {
                this._addWarning("Variable " + key + " is NEWed but not used", state.newedAtLine[0], state.newedAtPostion[0], key.length);
            }
        }
    }
    /**
     ...
     */
    _checkNewed(varName, level, line, position) {
     ...
     */
    _addWarning(message, line, startPosition, len, severity) {
    ...
    _checkLine(line, tokens) {
        ...
        for (let tokenId = 0; tokenId < tokens.length; tokenId++) { // iterate over every token in actual line
            ...
            if (token.type === mumpsLineParser_1.TokenType.comment && token.name.match(/ignoreVars:/)) { //Check for IgnoreVars-directive
                this._variablesToBeIgnored = this._variablesToBeIgnored.concat(token.name.split("ignoreVars:")[1].split(","));
            }
            if (tokenId === 0 && token.type === mumpsLineParser_1.TokenType.label) { //If there was unreachable code before this label
                //save a warning
                ...
                if (tokens[1] !== undefined && tokens[1].type === mumpsLineParser_1.TokenType.local) { //Begin of a parametrized subroutine
                    this._routine.startLine = line;
                    while (++tokenId < tokens.length && tokens[tokenId].type === mumpsLineParser_1.TokenType.local) {
            ...
            if (token.type === mumpsLineParser_1.TokenType.keyword || token.type === mumpsLineParser_1.TokenType.comment) { //Check intendation level
                ...
                if (!ifFlag && (command === "QUIT" || command === "GOTO" || command === "HALT") && !token.isPostconditioned && this._level === 0) {
                    ...
                    if (command === "GOTO") { //Check if GOTO argument is postconditioned
                        for (let k = tokenId + 1; k < tokens.length; k++) {
                            if (tokens[k].type === mumpsLineParser_1.TokenType.argPostcondition) {
                                hasPostcondition = true;
                                break;
                            }
                            else if (tokens[k].type === mumpsLineParser_1.TokenType.keyword) {
                                break;
                            }
                        }
                    }
            ...
            if (token.type === mumpsLineParser_1.TokenType.intendation) { //check if new intendation level is OK and remember new level
     ...
     */
    _reduceIntendationLevel(level, newLevel) {
        var _a;
        ...
        for (const key in this._varStates) {
            ...
            if (state.newedAtLevel) {
                for (let k = newLevel + 1; k <= level; k++) {
                    const index = (_a = state.newedAtLevel) === null || _a === void 0 ? void 0 : _a.indexOf(k);
                    if (index > -1) {
                ...
                if (found && !state.isUsed) {
                    this._addWarning("Variable " + key + " is NEWed but not used", memLine, memPosition, key.length);
                }
            }
        }
    }
    _addSymbol(name, startLine, endLine) {
...
exports.default = MumpsDiagnosticsProvider;
//# sourceMappingURL=mumpsDiagnosticsProvider.js.map